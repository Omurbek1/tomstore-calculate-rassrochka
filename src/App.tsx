...
function App() {
  ...
  const handleShare = async () => {
    if (!results) return;
    let text = `📱 *TomStore.kg: Расчет рассрочки*\n💰 Цена товара: ${productPrice} с\n💵 Взнос: ${initialPayment} с\n📉 *Сумма рассрочки: ${formatCurrency(loanAmount)} с*\n`;
    ...
    results
      ...
      .forEach((p) => {
        ...
        p.rows.forEach((r: any) => {
          ...
          // ВНИМАНИЕ: Здесь в text НЕ добавляется r.showRate (процент скрыт)
          text += `\n ${r.months} мес ${typeLabel}: ${formatCurrency(r.monthly)} /мес (Итого: ${formatCurrency(r.total)} с)`;
        });
      });
    ...
    if (
      ...
      navigator.clipboard.writeText(text).then(() => {
        setCopySuccess("✅ Расчет для клиента скопирован!");
    ...
    <div className="main-wrapper">
      ...
      <div className="container">
        <header className="header">
          ...
          <div className="status-badge">Панель менеджера</div>
        </header>
                        ...
                        {product.rows.map((row: any, idx: number) => (
                          ...
                          >
                            <td className="cell-term">
                              {/* НА ЭКРАНЕ: Менеджер видит и месяц, и процент */}
        ...
        <footer className="footer-info">
          ...
          <div className="footer-actions">
            <a href={`tel:${WHATSAPP_PHONE}`} className="btn-footer">
              📞 Позвонить
            </a>
            <a
      ...
      <style>{`
        ...
        .background-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(180deg, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.98) 100%); backdrop-filter: blur(10px); z-index: -1; }
        .container { max-width: 600px; margin: 0 auto; color: #fff; }
        .logo { font-size: 2.2em; font-weight: 800; color: #3b82f6; text-align: center; margin: 0; }
        .status-badge { text-align: center; font-size: 0.65em; color: #64748b; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 15px; }
        .glass-card { background: rgba(255, 255, 255, 0.07); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 20px; padding: 15px; margin-bottom: 15px; }
        ...
        .input-group input { width: 100%; background: #000; border: 1px solid #334155; padding: 12px; border-radius: 10px; color: #fff; font-size: 1.2em; font-weight: 700; box-sizing: border-box; outline: none; }
        .btn-main { width: 100%; padding: 14px; background: #2563eb; border: none; border-radius: 12px; color: #fff; font-weight: 800; cursor: pointer; }
        .summary-bar { display: flex; justify-content: space-between; align-items: center; }
        .loan-info .value { font-size: 1.4em; font-weight: 800; color: #3b82f6; }
        .btn-icon-text { background: #fff; color: #000; border: none; padding: 8px 15px; border-radius: 10px; font-weight: 800; font-size: 0.8em; }
        .tab-item { padding: 8px 18px; border-radius: 12px; background: #1e293b; border: none; color: #94a3b8; font-weight: 700; font-size: 0.75em; white-space: nowrap; margin-right: 5px; }
        .tab-item.active { background: #3b82f6; color: #fff; }
        .product-title { font-size: 0.9em; color: #60a5fa; margin: 0 0 10px 0; border-left: 3px solid #3b82f6; padding-left: 8px; }
        .results-table { width: 100%; border-collapse: collapse; }
        .results-table th { font-size: 0.55em; color: #64748b; text-transform: uppercase; padding-bottom: 8px; text-align: center; }
        .results-table td { padding: 12px 2px; border-top: 1px solid rgba(255, 255, 255, 0.05); text-align: center; font-size: 0.85em; }
        .cell-term { text-align: left !important; padding-left: 8px !important; }
        ...
        .cell-total { color: #94a3b8; font-size: 0.8em; }
        .cell-over { color: #ef4444; font-weight: 600; font-size: 0.8em; }
        .footer-info { text-align: center; font-size: 0.8em; color: #475569; margin-top: 15px; }
        .footer-actions { display: flex; gap: 10px; margin-top: 10px; padding-bottom: 20px; }
        .btn-footer { flex: 1; text-align: center; background: #334155; padding: 12px; border-radius: 12px; color: #fff; text-decoration: none; font-weight: 700; font-size: 0.85em; }
        .toast-success { text-align: center; color: #10b981; font-weight: 700; font-size: 0.85em; margin-bottom: 10px; }
      `}</style>
    </div>
  );
}
